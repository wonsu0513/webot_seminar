...
double* robot_controller(double* start_pos, double* target_pos) {
    ...
    double k_rho = 9;        //should be larger than 0, i.e, k_rho > 0
...
int main(int argc, char **argv)
{
   ...
   while (wb_robot_step(TIME_STEP) != -1) {
    ...
    // Read a goal position from the current mouse point
    ...
    if (mouse_state.left){    
	// When you push the left button of the mouse, this loop will be running to go to the goal position (= the current mouse point). 
